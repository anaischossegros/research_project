...
for fp in data_path.glob('*.tsv'):
    ...
    data = pd.read_csv(fp, sep="\t", names = ['gene_id',	'gene_name',	'gene_type',	'unstranded',	'stranded_first',	'stranded_second',	'tpm_unstranded',	'fpkm_unstranded',	'fpkm_uq_unstranded'], usecols = [8], skiprows = 6)
    datas.append(data)
...
output_df2= output_df.set_index(['filename'])
...
data_norm_path = Path("C:/Users/anais/Documents/2-Imperial/0-Research-Project/4-NN/Cox-nnet model/data_lung/normalized_lung_counts.txt")

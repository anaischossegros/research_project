...
#transpose the tab
x_df_norm = x_df.transpose()
x_df_norm = x_df_norm.assign(filename=datas_id)

output_dfnorm = output_df.transpose()
x_df_norm= x_df_norm.set_index(['filename'])

#remove cases without outputs
x_df_norm = x_df_norm.drop(missing, axis = 0)
output_dfnorm= output_dfnorm.set_index(['filename'])

#convert into csv
# x_df_norm.to_csv('x_dfraw.csv')
# output_dfnorm.to_csv('output_dfnorm.csv')
...
data_norm_path = Path("C:/Users/anais/Documents/2-Imperial/0-Research-Project/4-NN/Cox-nnet model/src/normalized_lung_counts.txt")
